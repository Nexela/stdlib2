...
--- @param arrays table_array[] An array of arrays to merge.
--- @return table_array
...
function table.deep_compare(tbl1, tbl2)
    ...
    for k in pairs(tbl2) do if tbl1[k] == nil then return false end end
    return true
end
...
function table.for_each(tbl, callback, ...)
    for k, v in pairs(tbl) do if callback(v, k, ...) then return true end end
    return false
end
...
--- @param list table_array
--- @param pos number
--- @param value any
--- @return boolean #true if the item was inserted. false if item was already in the list.
--- @overload fun(list: table_array, value: any):boolean
function table.unique_insert(list, pos, value)
    if value == nil then
        value = pos
        pos = #list + 1
    end
    for _, val in ipairs(list) do if val == value then return false end end
...
function table.shallow_merge(tables)
    local output = {}
    for _, tbl in pairs(tables) do for key, value in pairs(tbl) do output[key] = value end end
    return output
end
...
--- @return table_array #A new array with the copied values.
...
--- @param start int default: `1`
--- @param stop? int Stop at this index. If zero or negative, will stop `n` items from the end of the array (default: `#array`).
--- @return table_array #A new array with the extracted values.
